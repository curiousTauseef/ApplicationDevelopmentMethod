 ...
 */
@WebServlet(name = "CoffeeMachineController", urlPatterns = {"/CoffeeMachineController"})
public class CoffeeMachineController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        CoffeeMachine m = null;
        if (session == null) {
            session = request.getSession();
            m = new CoffeeMachine(1000, 1000);
            session.setAttribute("machine", m);
            session.setAttribute("message", "\"message\":\"\"");
        } else {
            m = (CoffeeMachine) session.getAttribute("machine");
        };
        ...
        json = "{" + session.getAttribute("message") + "," + session.getAttribute("machine") + "}";
